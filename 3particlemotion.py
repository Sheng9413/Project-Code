...
def GFU(F1,F2,x1,y1,x,y):
    ...
    r2 = y-y1
    r = np.sqrt( r1**2+r2**2)
    return (r**2*F1 + r1**2*F1+r1*r2*F2)/(r**(3)+0.0001)

def GFV(F1,F2,x1,y1,x,y):
    r1 = (x-x1)
    r2 = y-y1
    r = np.sqrt( r1**2+r2**2)
    return ( r**2*F2+r1*r2*F1+r2**2*F2) /(r**(3)+0.0001)
...
U1 = GFU(0,-1,x2,y2,x1,y1)  + GFU(0,-1,x3,y3,x1,y1)
V1 = GFV(0,-1,x2,y2,x1,y1)  + GFV(0,-1,x3,y3,x1,y1)

U2 = GFU(0,-1,x1,y1,x2,y2)  + GFU(0,-1,x3,y3,x2,y2)
V2 = GFV(0,-1,x1,y1,x2,y2)  + GFV(0,-1,x3,y3,x2,y2)

U3 = GFU(0,-1,x2,y2,x3,y3)  + GFU(0,-1,x1,y1,x3,y3)
V3 = GFV(0,-1,x2,y2,x3,y3)  + GFV(0,-1,x1,y1,x3,y3)
...
writer = animation.PillowWriter(fps=15,
                                ...
                                bitrate=1800)
ani.save('3particle.gif', writer=writer)
